...
int MessageManager::deserializeAction(const char * entry){
  json_error_t error;
  ...
  if(!action.compare("helloack")){
    ...
    json_decref(root);
    //setting up the first element in the queue
    //MIGHT HAVE TO DELETE OLD FIELD HERE
    vector<Moves> moves;
    Path* p = new Path(new Manager(_masterPtr), moves);
    q.push(p);
    return 1;
  }
  ...
  else if(!action.compare("requestmove")) {
    // proceed to send message in model's loop
    json_decref(root);
    return 2;
  } else {
    json_decref(root);
...
string MessageManager::serializeHello() {
  ...
  json_object_set_new(root, "teamname", json_string("Allen_Conner"));
...
string MessageManager::serializeState() {
  //cout << "here?"  << endl;
  int row1 = bestMove->getMoves().at(0).row;
  int col1 = bestMove->getMoves().at(0).col;
  int dir1 = bestMove->getMoves().at(0).dir;
  //cout << "no not here" << endl;
  // Apply move
  _masterPtr->setRow(row1);
  _masterPtr->setCol(col1);
  int ress = _masterPtr->searchBoard(dir1);
  //cout << "pkg eval " << row1 << ", " << col1 << ", " << dir1 << " = " << ress << endl;
  ...
  int row =_masterPtr->getCandyBoard()->_row - 1 - row1;
  json_object_set_new(move, "row", json_integer(row));
  json_object_set_new(move, "column", json_integer(col1));
  json_object_set_new(move, "direction", json_integer(dir1));
  json_object_set_new(root, "move", move);
  json_object_set_new(root, "movesevaluated", json_integer(movesExplored));
  // NEW INFO DONE. 
