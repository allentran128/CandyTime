...
void Manager::xSwap(int row, int col){
  int offset = this->_ext_offset[col] % this->_extColor->_row;
  //accesses a game piece in extended board to put in board
  *(int*)this->_candyboard->_arr[row * this->_candyboard->_col + col] = A2DintGet(this->_extColor->_row - offset -1,col,this->_extColor);
...
//takes in a parameter that represents which direction to swap
//left: 0, right: 1, up: 2, down:3, no direction: -1

int Manager::directionSwap(int direction){
  switch(direction){
    case -1:
      break;
    case 0 :
...
int Manager::searchVert(int interv){
  ...
  for(int k = 0; k < this->_candyboard->_col; k++){
    for(int i = this->_candyboard->_row-1; i >= interv-1; i--){
